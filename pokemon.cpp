...
int pokemon::get_pwr(int i){
  if((i<=1) && move_index[i]){
    return moves[move_index[i]].power;//getting power from csv file
  }
  else{
    return 0;
  }
...
int pokemon::get_acc(int i)
{
  return (int)ceil(moves[move_index[i]].accuracy);
}

int pokemon::get_dam(int i, int rand)
{
  int crit = 1;// calc crit later
  int type = 1;
  int STAB = 1; // calc stab later

  int power = moves[move_index[i]].power;
  if (power < 1)
  {
    power = 1;
  }

  double damage = (double)((double)(((double)((2 * level) / 5) + 2) *
				    power *((double)(effective_stat[stat_atk] /
						     effective_stat[stat_def]) / 50)) + 2) * crit *
                                                     (double)(rand / 100) * STAB * type;
